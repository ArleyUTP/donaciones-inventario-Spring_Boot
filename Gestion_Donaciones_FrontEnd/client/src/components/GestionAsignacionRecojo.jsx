...
function GestionAsignacionRecojo() {
    ...
    const cargarDatos = async () => {
        try {
            setLoading(true);
            const [asignacionesRes, voluntariosRes, donacionesRes] = await Promise.all([
                        ...
                        <DialogContent>
                            ...
                            <div className="grid gap-4 py-4">
                                <div className="grid gap-2">
                                    ...
                                    <Popover open={openVoluntario} onOpenChange={setOpenVoluntario}>
                                        <PopoverTrigger asChild>
                                            <Button variant="outline" role="combobox" aria-expanded={openVoluntario}>
                                                {selectedVoluntario ?
                                                    voluntarios.find(v => v.id.toString() === selectedVoluntario)?.usuario?.nombreUsuario
                                        ...
                                        <PopoverContent className="w-[300px] p-0">
                                            <Command>
                                                <CommandInput placeholder="Buscar voluntario..." className="h-9" />
                                                <CommandList>
                                                    <CommandEmpty>No se encontraron voluntarios.</CommandEmpty>
                                                    <CommandGroup>
                                                        {voluntarios.map((voluntario) => (
                                                            <CommandItem
                                                                ...
                                                                onSelect={(value) => {
                                                                    setSelectedVoluntario(value === selectedVoluntario ? "" : value);
                                                            ...
                                                            >
                                                                ...
                                                                <Check
                                                                    className={cn(
                                                                        "ml-auto h-4 w-4",
                                                                        selectedVoluntario === voluntario.id.toString() ? "opacity-100" : "opacity-0"
                                                                    )}
                                                                />
                                                            </CommandItem>
                                                        ))}
                                ...
                                <div className="grid gap-2">
                                    ...
                                    <Popover open={openDonacion} onOpenChange={setOpenDonacion}>
                                        <PopoverTrigger asChild>
                                            <Button variant="outline" role="combobox" aria-expanded={openDonacion}>
                                                {selectedDonacion ?
                                        ...
                                        <PopoverContent className="w-[300px] p-0">
                                            <Command>
                                                <CommandInput placeholder="Buscar donación..." className="h-9" />
                                                <CommandList>
                                                    <CommandEmpty>No hay donaciones pendientes.</CommandEmpty>
                                                    <CommandGroup>
                                                        {donaciones.map((donacion) => (
                                                            <CommandItem
                                                                ...
                                                                onSelect={(value) => {
                                                                    setSelectedDonacion(value === selectedDonacion ? "" : value);
                                                            ...
                                                            >
                                                                ...
                                                                <Check
                                                                    className={cn(
                                                                        "ml-auto h-4 w-4",
                                                                        selectedDonacion === donacion.id.toString() ? "opacity-100" : "opacity-0"
                                                                    )}
                                                                />
                                                            </CommandItem>
                                                        ))}
                            ...
                            <div className="flex justify-end space-x-2">
                                ...
                                <Button onClick={crearAsignacion}>
                                    Crear Asignación
                                </Button>
                            </div>
                        </DialogContent>
                                ...
                                <TableBody>
                                    {asignaciones.map((asignacion) => (
                                        <TableRow key={asignacion.id}>
                                            ...
                                            <TableCell>{asignacion.voluntario?.usuario?.nombreUsuario}</TableCell>
                                            <TableCell>
                                            ...
                                            <TableCell>
                                                <Badge
                                                    className={cn(
                                                        asignacion.estado === "Completada" ? "bg-green-500" :
                                                            asignacion.estado === "Pendiente" ? "bg-yellow-500" :
                                                                "bg-gray-500",
                                                        "text-white"
                                                    )}
                                                >
