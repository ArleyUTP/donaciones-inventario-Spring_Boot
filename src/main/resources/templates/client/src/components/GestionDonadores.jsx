...
function GestionDonadores() {
  ...
  useEffect(() => {
    const fetchTipoDonador = async () => {
      try {
        const response = await Axios.get(
          "http://localhost:3000/donorType/tiposDonador"
        );
        setTiposDonador(response.data);
      } catch (error) {
        console.error("Error al cargar tipos donador:", error);
        MySwal.fire({
          title: "Error",
          text: "No se pudieron cargar los tipos de donadores" + error,
          icon: "error",
        });
      }
    };
  ...
  const eliminarDonador = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        Axios.delete(`http://localhost:3000/donadores/delete/${id}`).then(
          () => {
            getDonadores();
            MySwal.fire({
              title: "Eliminado",
                        ...
                        >
                          ...
                          <td className="px-2 py-2 border-b border-blue-100 flex flex-col gap-1 sm:flex-row">
                            ...
                            <button
                              ...
                              onClick={() => eliminarDonador(val.donador_id)}
                            >
                              Eliminar
                            </button>
                          </td>
